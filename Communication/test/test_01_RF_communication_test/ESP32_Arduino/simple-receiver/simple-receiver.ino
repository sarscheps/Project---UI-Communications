...
void setup() {
  ...
  if (!radio.begin(RF24NT_RF_CHANNEL, RF24NT_PA_LEVEL, RF24NT_DATA_RATE, RF24NT_CRC_LENGTH, false)) {
    ...
    Serial.print(F("CE --> "));
    Serial.println(RF24NT_PIN_CE);
    Serial.print(F("CSN --> "));
    Serial.println(RF24NT_PIN_CSN);
  }
...
void loop() {
  ...
  if (radio.available(&pipNum)) {
    Serial.println("Start Reciving ...");

    radio.read(&payload, MAX_PAYLOAD_SIZE);

    Serial.print(F("Received payload from IP: "));
    Serial.println(payload.destination_IP, BIN);
    Serial.print(F("Sender IP: "));
    Serial.println(payload.local_IP, BIN);
